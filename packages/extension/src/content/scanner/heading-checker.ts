    ...
    if (fontSize >= 20 && fontWeight >= 700 && isBlock && directText.length < 120) {
      ...
      if (fakeHeadingCount <= 5) {
        ...
        findings.push({
          ...
          severity: "info",
          title: `Non-heading element styled as heading`,
...
export function checkHeadings(): HeadingResult {
  ...
  headingElements.forEach((el, index) => {
    ...
    if (!text) {
      findings.push({
        ...
        scanner: "heading-checker",
        severity: "warning",
        title: `Empty <${el.tagName.toLowerCase()}> heading`,
        description:
    ...
    if (hidden && !ariaHid && text) {
      findings.push({
        ...
        scanner: "heading-checker",
        severity: "info",
        title: `Visually hidden <${el.tagName.toLowerCase()}>`,
    ...
    if (ariaHid && text) {
      findings.push({
        ...
        scanner: "heading-checker",
        severity: "warning",
        title: `Heading inside aria-hidden region`,
  ...
  for (const heading of visibleHeadings) {
    if (previousLevel > 0 && heading.level > previousLevel + 1) {
      ...
      findings.push({
        ...
        scanner: "heading-checker",
        severity: "warning",
        title: `Heading level skipped: H${previousLevel} to H${heading.level}`,
  ...
  if (visibleHeadings.length > 0 && visibleHeadings[0].level !== 1) {
    findings.push({
      ...
      scanner: "heading-checker",
      severity: "info",
      title: `First heading is H${visibleHeadings[0].level}, not H1`,
      description:
